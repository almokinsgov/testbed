...
function getSttCorrectionProxyCandidates(word){
  const key = normalizeSttCorrectionWordKey(word);
  const out = [];
  if(key && STT_CORRECTION_DICT_STATE.loaded){
    ...
    if(ipas && ipas.size){
      for(const ipa of ipas){
        ...
        if(keys.length){
          out.push({ source:'cmudict', display:String(ipa || ''), keys });
        }
      }
    }
  }
  const heuristicKeys = getSttCorrectionHeuristicKeys(word);
  if(heuristicKeys.length){
    out.push({ source:'heuristic', display:heuristicKeys.join(' / '), keys:heuristicKeys });
  }
  return out;
}
...
function getSttCorrectionNearInfo(proxyKey, pos, segKey){
  ...
  if(remaining.length < segKey.length && remaining.length >= 2 && segKey.startsWith(remaining)){
    return { type:'partial', consume: remaining.length, matched: remaining };
  }
  ...
  function dfs(pos, gapsUsed){
    ...
    const startChar = proxyKey.charAt(pos);
    const bucket = lexiconState.byStart.get(startChar) || [];
    for(const item of bucket){
      let matchInfo = null;
      if(proxyKey.startsWith(item.key, pos)){
        matchInfo = { type:'exact', consume:item.key.length, matched:item.key };
      } else {
        matchInfo = getSttCorrectionNearInfo(proxyKey, pos, item.key);
      }
      if(!matchInfo) continue;
      const next = dfs(pos + matchInfo.consume, gapsUsed);
      const candidate = {
        matches: [{
          unit: item.unit,
    ...
    if(gapsUsed < maxGaps){
      const next = dfs(pos + 1, gapsUsed + 1);
      const candidate = {
  ...
  result.coverage = totalLen ? ((result.matchedChars || 0) / totalLen) : 0;
  result.reconstructed = (result.matches || []).map(x => x.unitNorm).join('');
  result.segments = (result.matches || []).map(x => x.unitNorm);
  return result;
}

function suggestSttCorrectionForWord(word){
  const lexiconState = getSttCorrectionLexiconState();
  const proxyCandidates = getSttCorrectionProxyCandidates(word);
  const results = [];
  for(const candidate of proxyCandidates){
    for(const key of (candidate.keys || [])){
      const match = searchSttCorrectionKey(key, lexiconState);
      if(!(match.matches || []).length) continue;
      results.push({
        source: candidate.source,
        displaySource: candidate.display,
        proxyKey: key,
        proxyPenalty: getSttCorrectionProxyPenalty(key),
...
function renderSpeechToText(){
  ...
  function drawCorrectionResults(items){
    ...
    for(const item of items){
      ...
      for(let ci = 0; ci < (item.candidates || []).length; ci++){
        const cand = item.candidates[ci];
  ...
  async function runCorrections(renderNow){
    ...
    if(dictState.ok){
      setCorrectionDictStatus(`Dictionary: ${dictState.entries.toLocaleString()} entries`, 'ok');
    } else {
      setCorrectionDictStatus(
        dictState.source === 'file-protocol-blocked'
    ...
    for(let i=0; i<(detectRes.scored || []).length; i++){
      ...
      const rankedCandidates = suggestSttCorrectionForWord(raw);
      ...
      items.push({
        ...
        trace: {
          ...
          minCoverage,
          proxyCandidates: getSttCorrectionProxyCandidates(raw),
  ...
  correctionDictBtn.addEventListener('click', async ()=>{
    ...
    if(state.ok) setCorrectionDictStatus(`Dictionary: ${state.entries.toLocaleString()} entries`, 'ok');
    else setCorrectionDictStatus(
      state.source === 'file-protocol-blocked'
        ? 'Dictionary: file:// auto-load blocked, heuristic only'
        : 'Dictionary: heuristic fallback only',
      'warn'
    );
