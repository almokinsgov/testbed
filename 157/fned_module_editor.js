...
(function () {
  ...
  function buildSitrepReportModel(data) {
    ...
    if (cfg.includeFndcTiming) {
      var timing = stats.fndcAlertTiming || {};
      sectionsById.fndcActive = {
        id: "fndcActive",
        title: "FNDC Alert Messages - Active",
        type: "table",
        ...
        rows: asArray(timing.active).map(function (item) {
          return [
            ...
            stripHtmlText(takeValues(item, ["alertType"])),
            formatSitrepDate(takeValues(item, ["publishDate"])),
            formatSitrepDate(takeValues(item, ["endDate"])),
            stripHtmlText(takeValues(item, ["description"]))
          ];
        })
      };
      sectionsById.fndcScheduled = {
        id: "fndcScheduled",
        title: "FNDC Alert Messages - Scheduled",
        type: "table",
        columns: ["Title", "Type", "Publish", "Ends", "Description"],
