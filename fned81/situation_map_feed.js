...
*/
   ...
   var layerLocalRoadClosures = L.layerGroup();
    ...
    var MARKER_STACK_REGISTRY = Object.create(null);

    function markerStackKey(lat, lng) {
      return Number(lat).toFixed(6) + "," + Number(lng).toFixed(6);
    }

    function applyMarkerStackOffset(lat, lng, registry) {
      if (!isFinite(lat) || !isFinite(lng)) {
        return [lat, lng];
      }
      var targetRegistry = registry || MARKER_STACK_REGISTRY;
      var key = markerStackKey(lat, lng);
      var count = targetRegistry[key] || 0;
      targetRegistry[key] = count + 1;
      if (!count) {
        return [lat, lng];
      }
    ...
    function addClusteredSymbolMarkers(records, targetLayer, options) {
      ...
      Object.keys(byType).forEach(function (typeKey) {
        ...
        groups.forEach(function (group) {
          ...
          var marker;
          if (group.items.length >= minCount) {
...
function renderDynamicClusterState(key) {
  ...
  addClusteredSymbolMarkers(state.records || [], state.targetLayer, {
    ...
    stackRegistry: Object.create(null)
  });
}

function renderAllDynamicClusterStates() {
  Object.keys(DYNAMIC_CLUSTER_STATES).forEach(renderDynamicClusterState);
}
