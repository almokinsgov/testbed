...
// Depends on app_shared.js, segmenter_core.js, and later inline helpers such as tokenizeTextBlockParts.
  ...
  function decodeXmlEntitiesText(s){
    return String(s || '')
      .replace(/&quot;/g, '"')
      .replace(/&#39;/g, "'")
      .replace(/&lt;/g, '<')
  ...
  function applyCompactSsmlPostProcessText(ssmlText, opts={}){
    ...
    function flushIpa(){
      ...
      if(ipaOnly){
        ...
        out += `<phoneme alphabet="ipa" ph="${escapeXmlAttrText(bufIpa)}">${bodyText}</phoneme>`;
      }
      bufIpa = '';
    }
    ...
    while((m = phonemeRe.exec(body)) !== null){
      ...
      appendIpa(decodeXmlEntitiesText(m[2] || ''));
  ...
  function renderG2PV4Lexicon(){
    ...
    runBtn.addEventListener('click', async ()=>{
      const rawText = String(input.value || '').normalize('NFC').trim();
      if(!rawText){
        outWrap.innerHTML = '';
  ...
  function renderPublicDemoTabVariant(cfg){
    ...
    let compactEnableInput = null;
    let compactIpaOnlyInput = null;
    let compactSentenceInput = null;
    let compactNewlineInput = null;
    let compactSpaceInput = null;
    let compactEnsureSpaceInput = null;
    let compactTeReoOnlyInput = null;
    let compactEmptyBodyInput = null;
    ...
    let autoAmbInfluenceInput = null;
    let autoParticleInfluenceInput = null;
    ...
    if(includeAwsControls){
      ...
      compactEnableInput = el('input', {type:'checkbox', name:idPrefix + 'SsmlCompactEnabled', 'aria-label':'Enable SSML compact mode'});
      compactEnableInput.checked = !!awsPrefs.ssmlCompactEnabled;
      compactIpaOnlyInput = el('input', {type:'checkbox', name:idPrefix + 'SsmlCompactIpaOnly', 'aria-label':'Output IPA only'});
      compactIpaOnlyInput.checked = !!awsPrefs.ssmlCompactIpaOnly;
      compactSentenceInput = el('input', {type:'checkbox', name:idPrefix + 'SsmlCompactSplitSentence', 'aria-label':'Split at sentence punctuation'});
      compactSentenceInput.checked = awsPrefs.ssmlCompactSplitSentence !== false;
      compactNewlineInput = el('input', {type:'checkbox', name:idPrefix + 'SsmlCompactSplitNewline', 'aria-label':'Split at newlines'});
      compactNewlineInput.checked = awsPrefs.ssmlCompactSplitNewline !== false;
      compactSpaceInput = el('input', {type:'checkbox', name:idPrefix + 'SsmlCompactSplitSpace', 'aria-label':'Split at spaces'});
      compactSpaceInput.checked = !!awsPrefs.ssmlCompactSplitSpace;
      compactEnsureSpaceInput = el('input', {type:'checkbox', name:idPrefix + 'SsmlCompactEnsureSpace', 'aria-label':'Add space between IPA chunks'});
      compactEnsureSpaceInput.checked = awsPrefs.ssmlCompactEnsureSpace !== false;
      compactTeReoOnlyInput = el('input', {type:'checkbox', name:idPrefix + 'SsmlCompactTeReoOnly', 'aria-label':'Apply compact to te reo blocks only'});
      compactTeReoOnlyInput.checked = !!awsPrefs.ssmlCompactTeReoBlocksOnly;
      compactEmptyBodyInput = el('input', {type:'checkbox', name:idPrefix + 'SsmlCompactEmptyBody', 'aria-label':'Use empty phoneme body in compact output'});
      compactEmptyBodyInput.checked = awsPrefs.ssmlCompactEmptyBody !== false;
      ...
      const autoWordDetailsCollapse = el('details', {class:'tokenDetails', style:'margin-top:12px;'});
      autoWordDetailsCollapse.open = false;
      autoWordDetailsCollapse.appendChild(el('summary', {}, ['Word Details']));
      autoWordDetailsCollapse.appendChild(autoWordWrap);
      autoDetectPanel.appendChild(autoWordDetailsCollapse);
    ...
    if(includeAwsControls){
      ...
      const tokenListsPanel = el('div', {class:'card', style:'margin-top:10px;'});
      tokenListsPanel.appendChild(el('h3', {}, ['Token Lists (Editable)']));
      tokenListsPanel.appendChild(el('p', {class:'muted'}, ['Edit ambiguous and particle tokens used by Auto Detect Te Reo. Use commas or new lines.']));
      ...
      tokenListsPanel.appendChild(tokenEditors);
      ...
      const ambInfluenceTag = el('span', {class:'tag'}, ['Ambiguous Influence: ' + (((influencePrefs.autoDetectAmbiguousInfluence ?? 1) * 100).toFixed(0)) + '%']);
      autoAmbInfluenceInput = el('input', {type:'range', min:'0', max:'2', step:'0.05', value:String(influencePrefs.autoDetectAmbiguousInfluence ?? 1), style:'width:240px;', 'aria-label':'Ambiguous token influence'});
      ambInfluenceWrap.appendChild(ambInfluenceTag);
      ambInfluenceWrap.appendChild(autoAmbInfluenceInput);

      const particleInfluenceWrap = el('div', {style:'min-width:260px;'});
      const particleInfluenceTag = el('span', {class:'tag'}, ['Particle Influence: ' + (((influencePrefs.autoDetectParticleInfluence ?? 1) * 100).toFixed(0)) + '%']);
      autoParticleInfluenceInput = el('input', {type:'range', min:'0', max:'2', step:'0.05', value:String(influencePrefs.autoDetectParticleInfluence ?? 1), style:'width:240px;', 'aria-label':'Particle token influence'});
      particleInfluenceWrap.appendChild(particleInfluenceTag);
      particleInfluenceWrap.appendChild(autoParticleInfluenceInput);

      influenceRow.appendChild(ambInfluenceWrap);
      influenceRow.appendChild(particleInfluenceWrap);
      tokenListsPanel.appendChild(influenceRow);
      ...
      tokenListsPanel.appendChild(tokenBtnRow);

      function saveInfluencePrefs(){
        const amb = Math.max(0, Math.min(2, parseFloat((autoAmbInfluenceInput && autoAmbInfluenceInput.value) || '1') || 1));
        const par = Math.max(0, Math.min(2, parseFloat((autoParticleInfluenceInput && autoParticleInfluenceInput.value) || '1') || 1));
      ...
      autoAmbInfluenceInput.addEventListener('input', ()=>{
        saveInfluencePrefs();
        generateAwsSsmlFromCurrent();
      });
      autoParticleInfluenceInput.addEventListener('input', ()=>{
        saveInfluencePrefs();
        generateAwsSsmlFromCurrent();
      });
      ...
      awsControlCard.appendChild(tokenListsPanel);
      awsControlCard.appendChild(mixedWrapPanel);

      compactCard = el('div', {class:'card'});
      compactCard.appendChild(el('h3', {}, ['SSML Compact Post Processing']));
      compactCard.appendChild(el('p', {class:'muted'}, ['Optional post-processing to reduce AWS SDK character usage by compacting phoneme output and preserving punctuation boundaries.']));
      const compactRowA = el('div', {class:'row', style:'gap:8px; flex-wrap:wrap;'});
      const compactRowB = el('div', {class:'row', style:'margin-top:10px; gap:8px; flex-wrap:wrap;'});
      const compactRowC = el('div', {class:'row', style:'margin-top:10px; gap:8px; flex-wrap:wrap;'});
      for(const pair of [
        ['Enable SSML Compact Mode', compactEnableInput],
        ['Output IPA Only (No SSML)', compactIpaOnlyInput]
      ]){
        const label = el('label', {class:'checkbox'});
        label.appendChild(pair[1]);
        label.appendChild(document.createTextNode(pair[0]));
        compactRowA.appendChild(label);
      }
      for(const pair of [
        ['Split At Sentence Punctuation', compactSentenceInput],
        ['Split At Newlines', compactNewlineInput],
        ['Split At Spaces', compactSpaceInput]
      ]){
        const label = el('label', {class:'checkbox'});
        label.appendChild(pair[1]);
        label.appendChild(document.createTextNode(pair[0]));
        compactRowB.appendChild(label);
      }
      for(const pair of [
        ['Add Space Between Adjacent IPA Chunks', compactEnsureSpaceInput],
        ['Apply Compact To Te Reo Blocks Only', compactTeReoOnlyInput],
        ['Use Empty Phoneme Body In Compact Output', compactEmptyBodyInput]
      ]){
        const label = el('label', {class:'checkbox'});
        label.appendChild(pair[1]);
        label.appendChild(document.createTextNode(pair[0]));
        compactRowC.appendChild(label);
      }
      ...
      const ssmlMapDetails = el('details', {class:'tokenDetails', style:'margin-top:12px;'});
      ssmlMapDetails.open = false;
      ssmlMapDetails.appendChild(el('summary', {}, ['Mapping Table']));
      ssmlMapWrap = el('div', {style:'margin-top:10px;'});
      ssmlTraceWrap = el('div', {style:'margin-top:12px;'});
      ssmlMapDetails.appendChild(ssmlMapWrap);
      ssmlCard.appendChild(ssmlMapDetails);
    ...
    if(compactCard) root.appendChild(compactCard);
    ...
    function getPublicDemoAnalysisTokens(rawText){
      const raw = String(rawText || '').normalize('NFC');
    ...
    function getPublicAutoDetectInfluenceOptions(){
      const prefs = loadG2pAwsTtsPrefs();
      const amb = Math.max(0, Math.min(2, parseFloat((autoAmbInfluenceInput && autoAmbInfluenceInput.value) || prefs.autoDetectAmbiguousInfluence || '1') || 1));
      const par = Math.max(0, Math.min(2, parseFloat((autoParticleInfluenceInput && autoParticleInfluenceInput.value) || prefs.autoDetectParticleInfluence || '1') || 1));
      return {
        ambiguousInfluence: amb,
        particleInfluence: par
      };
    }

    function getAutoWrapMap(rawText){
      syncPublicAutoDetectState(rawText);
      const detect = computeDetectForStats(
        String(rawText || ''),
        clamp01(parseFloat((autoThr && autoThr.value) || '0.75')),
        getPublicAutoDetectInfluenceOptions()
      );
    ...
    function renderAutoDetectPreview(rawText){
      ...
      for(const tok of tokens){
        ...
        span.addEventListener('click', (ev)=>{
          ...
          publicAutoDetectState.selectedKey = key;
          renderAutoDetectPreview(String(input.value || '').normalize('NFC'));
        });
    ...
    function trimPublicDemoIpaPart(ipaPart, opts={}){
    ...
    function trimPublicDemoIpa(wordObjOrIpa){
      ...
      const trimmedParts = parts.map((part, idx)=> trimPublicDemoIpaPart(part, { trimTail: idx < (parts.length - 1) }));
    ...
    function saveAwsControlPrefs(){
      ...
      saveG2pAwsTtsPrefs({
        ...
        ssmlCompactEnabled: !!(compactEnableInput && compactEnableInput.checked),
        ssmlCompactIpaOnly: !!(compactIpaOnlyInput && compactIpaOnlyInput.checked),
        ssmlCompactSplitSentence: !!(compactSentenceInput && compactSentenceInput.checked),
        ssmlCompactSplitNewline: !!(compactNewlineInput && compactNewlineInput.checked),
        ssmlCompactSplitSpace: !!(compactSpaceInput && compactSpaceInput.checked),
        ssmlCompactEnsureSpace: !!(compactEnsureSpaceInput && compactEnsureSpaceInput.checked),
        ssmlCompactTeReoBlocksOnly: !!(compactTeReoOnlyInput && compactTeReoOnlyInput.checked),
        ssmlCompactEmptyBody: !!(compactEmptyBodyInput && compactEmptyBodyInput.checked),
        ...
        autoDetectAmbiguousInfluence: Math.max(0, Math.min(2, parseFloat((autoAmbInfluenceInput && autoAmbInfluenceInput.value) || '1') || 1)),
        autoDetectParticleInfluence: Math.max(0, Math.min(2, parseFloat((autoParticleInfluenceInput && autoParticleInfluenceInput.value) || '1') || 1)),
    ...
    function generateAwsSsmlFromCurrent(){
      ...
      const rawText = String(input.value || '').normalize('NFC');
      const tokens = getPublicDemoAnalysisTokens(rawText);
      ...
      const baseSsml = (speakInput && speakInput.checked) ? ('<speak>' + ssmlBody + '</speak>') : ssmlBody;
      const teReoBlocksOnly = !!(compactTeReoOnlyInput && compactTeReoOnlyInput.checked);
      const compactAllowedByMode = !teReoBlocksOnly || outputMode === 'mixed' || outputMode === 'mixedAuto';
      const useCompact = (!!(compactEnableInput && compactEnableInput.checked) || !!(compactIpaOnlyInput && compactIpaOnlyInput.checked)) && compactAllowedByMode;
      let nextSsml = baseSsml;
      if(useCompact){
        try {
          nextSsml = applyCompactSsmlPostProcessText(baseSsml, {
            splitSentence: !!(compactSentenceInput && compactSentenceInput.checked),
            splitNewline: !!(compactNewlineInput && compactNewlineInput.checked),
            splitSpace: !!(compactSpaceInput && compactSpaceInput.checked),
            ensureSpace: !!(compactEnsureSpaceInput && compactEnsureSpaceInput.checked),
            ipaOnly: !!(compactIpaOnlyInput && compactIpaOnlyInput.checked),
            emptyBody: !!(compactEmptyBodyInput && compactEmptyBodyInput.checked),
            wrapSpeak: !!(speakInput && speakInput.checked) && !(compactIpaOnlyInput && compactIpaOnlyInput.checked)
          });
          if(nextSsml == null || nextSsml === '') nextSsml = baseSsml;
        } catch(_e){
          nextSsml = baseSsml;
        }
      }
      ssmlOut.value = nextSsml;
      ...
      if(ssmlTraceWrap){
        ...
        if(traceInput.checked){
          const lines = [
            ...
            'trimNonFinalH: ' + !!(trimNonFinalHInput && trimNonFinalHInput.checked),
            'removeLowerArticulation: ' + !!(removeLowerArticulationInput && removeLowerArticulationInput.checked),
            'compactMode: ' + !!(compactEnableInput && compactEnableInput.checked),
            'compactIpaOnly: ' + !!(compactIpaOnlyInput && compactIpaOnlyInput.checked),
            'compactSplitSentence: ' + !!(compactSentenceInput && compactSentenceInput.checked),
            'compactSplitNewline: ' + !!(compactNewlineInput && compactNewlineInput.checked),
            'compactSplitSpace: ' + !!(compactSpaceInput && compactSpaceInput.checked),
            'compactEnsureSpace: ' + !!(compactEnsureSpaceInput && compactEnsureSpaceInput.checked),
            'compactTeReoBlocksOnly: ' + !!(compactTeReoOnlyInput && compactTeReoOnlyInput.checked),
            'compactEmptyBody: ' + !!(compactEmptyBodyInput && compactEmptyBodyInput.checked),
            'strictMissing: ' + !!(strictInput && strictInput.checked)
          ];
          if(outputMode === 'mixedAuto'){
            lines.push('autoDetectThreshold: ' + Math.round(clamp01(parseFloat((autoThr && autoThr.value) || '0.75')) * 100) + '%');
            lines.push('autoDetectAmbiguousInfluence: ' + (Math.max(0, Math.min(2, parseFloat((autoAmbInfluenceInput && autoAmbInfluenceInput.value) || '1') || 1)).toFixed(2)));
            lines.push('autoDetectParticleInfluence: ' + (Math.max(0, Math.min(2, parseFloat((autoParticleInfluenceInput && autoParticleInfluenceInput.value) || '1') || 1)).toFixed(2)));
          }
    ...
    function buildWordCard(wordObj, wordIndex){
      ...
      if(modernSimpleLayout){
        ...
        const outputsCollapse = el('details', {class:'tokenDetails', style:'margin-top:10px;'});
        outputsCollapse.open = false;
        outputsCollapse.appendChild(el('summary', {}, ['Word Outputs: ' + String(wordObj.word || wordObj.wordNorm || ('#' + (wordIndex + 1)))]));
        outputsCollapse.appendChild(outputGroup);
        wordCard.appendChild(outputsCollapse);
    ...
    function renderAnalysis(analysis){
      ...
      if(resultsTitle){
        resultsCard = el('div', {class:'card' + (modernSimpleLayout ? ' publicSimpleSectionIntro' : '')});
      ...
      let wordOutputsHost = outWrap;
      if(modernSimpleLayout){
        const wordOutputsContainer = el('div', {class:'publicSimpleWordOutputs'});
        if(resultsCard){
          const outputsCollapse = el('details', {class:'tokenDetails', style:'margin-top:10px;'});
          outputsCollapse.open = false;
          outputsCollapse.appendChild(el('summary', {}, ['Word Outputs']));
          outputsCollapse.appendChild(wordOutputsContainer);
          resultsCard.appendChild(outputsCollapse);
        } else {
          outWrap.appendChild(wordOutputsContainer);
        }
        wordOutputsHost = wordOutputsContainer;
      }

      const words = analysis.words || [];
      for(let i=0;i<analysis.summary.rendered;i++) {
        wordOutputsHost.appendChild(buildWordCard(words[i], i));
      }
    }
