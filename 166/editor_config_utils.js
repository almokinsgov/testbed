(function () {
  ...
  var KNOWN_MAP_LAYER_DRAW_ORDER = [
  ...
  function normaliseLayerDrawOrder(order) {
    ...
    KNOWN_MAP_LAYER_DRAW_ORDER.forEach(function (name) {
      if (seen[name]) return;
      seen[name] = true;
      result.push(name);
    });
  ...
  function ensureConfigShape(raw) {
    ...
    data.runtimeConfig.map.connectionSettings = asObject(data.runtimeConfig.map.connectionSettings);
    [
      { key: "metServiceAtom", source: "https://raw.githubusercontent.com/almokinsgov/NZSHAPE/refs/heads/main/alerts/latest.xml", proxy: true },
      { key: "civilDefenceAtom", source: "https://www.civildefence.govt.nz/home/rss", proxy: true },
      { key: "topEnergyOutages", source: "https://outages.topenergy.co.nz/api/outages", proxy: true },
      { key: "topEnergyRegions", source: "https://outages.topenergy.co.nz/api/outages/regions", proxy: false },
      { key: "topEnergyKmz", source: "https://outages.topenergy.co.nz/storage/kmz/polygonsActiveAll.kmz", proxy: true },
      { key: "nztaDelays", source: "https://www.journeys.nzta.govt.nz/assets/map-data-cache/delays.json", proxy: true },
      { key: "localRoadClosures", source: "https://raw.githubusercontent.com/almokinsgov/FNDC_closures/main/public/public_closures_FNDC.json", proxy: true },
      { key: "councilAlerts", source: "https://raw.githubusercontent.com/almokinsgov/FNDC_closures/main/fned_custom_messages_example.json", proxy: false },
      { key: "geonetEarthquakes", source: "https://wfs.geonet.org.nz/geonet/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=geonet:quake_search_v1&outputFormat=json&cql_filter=depth%3C50+AND+origintime%3E=%272025-01-01%27+AND+DWITHIN(origin_geom,Point+(173.5+-35.2),140000,meters)", proxy: true },
      { key: "geonetCapFeed", source: "https://raw.githubusercontent.com/almokinsgov/NZSHAPE/refs/heads/main/Testing/GeoNet/quake-example.xml", proxy: true }
    ].forEach(function (def) {
      data.runtimeConfig.map.connectionSettings[def.key] = asObject(data.runtimeConfig.map.connectionSettings[def.key]);
      data.runtimeConfig.map.connectionSettings[def.key].sourceUrl = asString(
        data.runtimeConfig.map.connectionSettings[def.key].sourceUrl,
        def.source
      );
      data.runtimeConfig.map.connectionSettings[def.key].useProxy = asBoolean(
        data.runtimeConfig.map.connectionSettings[def.key].useProxy,
        def.proxy
      );
    });
