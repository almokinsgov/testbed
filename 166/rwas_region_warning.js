...
async function fetchAlerts() {
  try {
    ...
    for (const url of links) {
      ...
      const capIdentifier = capXML.getElementsByTagNameNS(capNS, "identifier")[0]?.textContent || url || "";
...
<div class="alert-card ${qualifies ? 'far-north' : ''} ${alertStatus} ${colourClass}">
  ...
  ${alertStatus === 'expired' ? `<span class="badge expired-badge">Expired</span>` : ''}
      ...
      if (qualifies) {
        ...
        console.warn("Skipping MetService CAP entry due to load/parse error", url, capErr);
      }
    }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (!contentXML) {
        ...
        if (altHref) {
          contentXML = await rwasFetchTextTracked("rwas.civil_defence.cap", proxy + encodeURIComponent(altHref));
        }
      }
...
${isCancelled ? `<span class="badge cancelled-badge">Cancelled</span><br>` : ''}
  <br><a href="https://www.civildefence.govt.nz/" target="_blank">Visit civildefence.govt.nz for more information</a>
      ...
      if (qualifies) {
        ...
        console.warn("Skipping Civil Defence entry due to load/parse error", entryErr);
      }
    }
